...
const ImageUpload = () => {
  ...
  const pictureState = usePictures();

  const handleImageChange = (e) => {

    const files = Array.from(e.target.files);
    pictureState.setPictures([...pictureState.pictures, ...files]);
    
  };

  const handleRemoveImage = (index) => {
    const newImages = pictureState.pictures.filter((_, i) => i !== index);
    pictureState.setPictures(newImages);
  };
    ...
    <div className="">
      ...
      <div className="rounded-lg p-4 mb-4 bg-white ">
        ...
        <div className="flex flex-wrap gap-4 m-4">
          {pictureState.pictures.map((image, index) => (
